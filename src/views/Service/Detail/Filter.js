...
import {
  changeFilter_serviceDetail,
  getList_serviceDetail,
  getBasicShop,
  getBasicCompany,
    ...
    return (
      <SRow gutter={16} className="">
        ...
        <SCol span={9}>
          ...
          <CSVLink data={formatCSV(list, header)} filename="服务费用明细表.csv">
            <Button>导出</Button>
          </CSVLink>
        </SCol>
      </SRow>
...
const cFilter = connect(
  ({ service, basic }) => ({
    filter: service.detailFilter,
    list: service.detailList,
    basic,
  }),
  {
    changeFilter: changeFilter_serviceDetail,
