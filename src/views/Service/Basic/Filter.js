...
import {
  changeFilter_serviceBasic,
  getList_serviceBasic,
  getBasicShop,
  getBasicCompany,
    ...
    return (
      <SRow gutter={16} className="">
        ...
        <SCol span={9}>
          ...
          <CSVLink data={formatCSV(list, header)} filename="服务费用统计表.csv">
            <Button>导出</Button>
          </CSVLink>
        </SCol>
      </SRow>
...
const cFilter = connect(
  ({ service, basic }) => ({
    filter: service.basicFilter,
    list: service.basicList,
    basic,
  }),
  {
    changeFilter: changeFilter_serviceBasic,
